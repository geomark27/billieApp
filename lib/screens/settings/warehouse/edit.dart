...
class EditWarehouseScreen extends StatefulWidget {
  const EditWarehouseScreen({super.key, required this.id});
...
class _EditWarehouseScreenState extends State<EditWarehouseScreen> {
  ...
  void fetchData(BuildContext context, int index) {
    setState(() {
      warehouse = context.read<CommonDataProvider>().wareHouses[index];
      nameController.text = warehouse.name;
      phoneController.text = warehouse.phone ?? "";
      emailController.text = warehouse.email;
  ...
  Future<void> handleSubmit() async {
    ...
    message = await WarehouseAPI.update(
      warehouse.id,
      Warehouse(
        id: warehouse.id,
