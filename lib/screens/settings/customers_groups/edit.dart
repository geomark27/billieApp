...
class EditCustomerGroupScreen extends StatefulWidget {
  const EditCustomerGroupScreen({super.key, required this.id});

  final int id;

  @override
  State<EditCustomerGroupScreen> createState() =>
      _EditCustomerGroupScreenState();
}

class _EditCustomerGroupScreenState extends State<EditCustomerGroupScreen> {
  late CustomerGroup customerGroup;
  ...
  void fetchData(BuildContext context, int index) {
    setState(() {
      customerGroup = context.read<CommonDataProvider>().customerGroups[index];
      nameController.text = customerGroup.name;
      percentageController.text = customerGroup.percentage;
    });
  }
  ...
  Future<void> handleSubmit() async {
    ...
    message = await CustomerGroupAPI.update(
      customerGroup.id,
      CustomerGroup(
        id: customerGroup.id,
